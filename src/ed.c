...
ssize_t blkio_read(int fd, void *buf, size_t count);
ssize_t blkio_write(int fd, void *buf, size_t count);
...
char * getblock(int atl, int iof){
	...
	if (iof==READ) {
		if (ichanged) {
			...
			blkio(iblock, ibuff, blkio_write);
		}
		ichanged = 0;
		iblock = bno;
		blkio(bno, ibuff, blkio_read);
		if(xtflag)
			crblock(tperm, ibuff, 512, (long)0);
		return(ibuff+off);
	}
	if (oblock>=0) {
		if(xtflag) {
			...
			blkio(oblock, crbuf, blkio_write);
		} else
			blkio(oblock, obuff, blkio_write);
	}
	oblock = bno;
	return(obuff+off);
}
